...
interface TodoItemProps {
    ...
    onTogle: (id: number, isCompleted: boolean) => void,
...
function TodoItem({ todo, deleteTodo, onTogle, onEdit }: TodoItemProps) {
    ...
    function togleTask(id: number, isCompleted: boolean) {

        onTogle(id, isCompleted)
    }

    function editTodo(id: number, title: string) {
        todo.id && todo.title && onEdit(todo.id, todo.title)
    }
    ...
    return (
        <li
            ...
            <label
                ...
                onClick={() => todo.id && togleTask(todo.id, todo.isCompleted)}
            ...
            <div className="todo__item-btns ml-auto flex">
                <button
                    onClick={() => todo.id && todo.title && editTodo(todo.id, todo.title)}
                ...
                <button
                    onClick={() => window.confirm("Do you really want to delete this task?") && todo.id && delTodo(todo.id)}
